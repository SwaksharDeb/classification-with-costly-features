...
DATA_FILE = "../data/stroke_rehabilitation-test"
...
MAX_MASK_CONST = 1.e6
CHUNK_SIZE = 100  # sample size or each batch contain this much samples
...
class PerfAgent():
    def __init__(self, env, brain):
        ...
        self.agents = self.env.agents # sample size of a perticular batch
        ...
        self.predicted_labels_batch = np.zeros(self.agents, dtype=np.int32)  # predicted labels of each batch
    ...
    def step(self):
        ...
        self.predicted_labels_batch[np.where(newly_finished==True)[0]] = a[np.where(newly_finished==True)[0]]
    ...
    def run(self):
        ...
        return avg_r, avg_len, avg_corr, self.predicted_labels_batch
...
class PerfEnv:
    def __init__(self, data_x, data_y, costs, ff):
        """
        Params:
            data_x = batch(contain 10 data samples) of input features
            data_y = batch(contain 10 data samples) of labels
            costs = cost of seeing the feature
        """
        self.x = data_x
        self.y = data_y
...
data_y   = data.iloc[:,   -2].astype('int32').values  # true labels
data_len = len(data)
predicted_labels = np.zeros(len(data_y), dtype=np.int32)
...
while idx < data_len:
    ...
    _r, _len, _corr, batch_prediction = agent.run()
    avg_r += _r
    avg_len += _len
    avg_corr += _corr
